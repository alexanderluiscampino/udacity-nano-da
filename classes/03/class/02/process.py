...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        rows = soup.select('tr[class=dataTDRight]')
        for row in rows:
            cells = [c for c in row.children if isinstance(c, Tag)]

            if not cells[1].string == 'TOTAL':
                info = {}
                info["courier"], info["airport"] = f[:6].split("-")
                info['year'] = int(cells[0].string)
                info['month'] = int(cells[1].string)
                info['flights'] = {
                    'domestic': int(locale.atof(cells[2].string)),
                    'international': int(locale.atof(cells[3].string))
                }
                data.append(info)
