...
def is_name_empty_or_bad(s):
    return s == 'NULL' or re.search('\W', s)
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            row['label'] = cleanup_label(row['label'])
            if is_name_empty_or_bad(row['name']):
                row['name'] = row['label']
            row = {k: convert_null(v) for k, v in row.items()}
            row['synonym'] = convert_array(row['synonym'])

            new_row = {
                'classification': {}
            }
            for k in row.keys():
                if k in ['synonym', 'name', 'uri', 'label', 'description']:
                    new_row[k] = row[k]
                else:
                    new_row['classification'][k] = row[k]
